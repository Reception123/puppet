...
class RenameBot:
    ...
    def get_pending_renames(self) -> list[dict]:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if m:
                ...
                if rid not in seen:
                    seen.add(rid)
                    items.append({"id": rid, "old_name": "", "new_name": ""})
    ...
    def fetch_request_details(self, entry: dict) -> dict:
        log = logging.getLogger("rename_bot")
        rid = entry["id"]
        r = self.session.get(f"{BASE_URL}/wiki/Special:GlobalRenameQueue/request/{rid}")
        r.raise_for_status()
        soup = BeautifulSoup(r.text, "html.parser")
        ...
        m = re.search(r"(\S+)\s+is requesting a rename to\s+(\S+)", page_text, re.IGNORECASE)
        if m:
        ...
        for sel in (".mw-message-box-warning", ".mw-message-box-error", ".mw-message-box"):
    ...
    def _approve(self, entry: dict) -> bool:
        ...
        if self.dry_run:
            log.info("[DRY-RUN] Would approve #%s: '%s' -> '%s'", rid, old_name, new_name)
            return True
        ...
        def is_approval_form(f):
            ...
            for el in f.find_all(["input", "button"]):
                ...
                if n in ACCEPT_NAMES or v in ACCEPT_TEXT or n in REJECT_NAMES or v in REJECT_TEXT:
                    return True
            return False
        ...
        fields["wpReason"]  = f"Request #{rid}: no concerns"
        fields["wpComment"] = "Automatically approved"
        ...
        if approve_btn:
            ...
            bval  = approve_btn.get("value") or approve_btn.get_text(strip=True) or "1"
            if bname:
                fields[bname] = bval
            for el in form.find_all(["input", "button"]):
                elname = (el.get("name") or "").lower()
                elval  = (el.get("value") or el.get_text(strip=True) or "").lower()
        ...
        if not action:
            action = f"{BASE_URL}/wiki/Special:GlobalRenameQueue/request/{rid}"

        r = self.session.post(action, data=fields)
        r.raise_for_status()
        page_text = BeautifulSoup(r.text, "html.parser").get_text().lower()

        if any(s in page_text for s in ("approved", "has been approved", "rename-queue-status-approved")):
            log.info("✓ Approved #%s: '%s' -> '%s'", rid, old_name, new_name)
            return True

        log.warning("Approval result for #%s unclear. Snippet:\n%s", rid, page_text[:500])
        return False
    ...
    def run(self):
        ...
        for entry in entries:
            ...
            try:
                ...
                log.error("Could not fetch #%s: %s", entry["id"], exc)
                continue

            log.info("  '%s' -> '%s'", entry.get("old_name") or "?", entry.get("new_name") or "?")

            skip, reason = self._should_skip(entry)
            if skip:
                log.warning("  Skipping (human review needed): %s", reason)
                continue

            if not self._approve(entry):
                log.error("  Approval failed for #%s.", entry["id"])
...
def load_config(path: str) -> dict:
    ...
    if not pw_path.exists():
        ...
        sys.exit(1)
    config["password"] = pw_path.read_text(encoding="utf-8").strip()
    return config


def main():
    parser = argparse.ArgumentParser(description="Miraheze global rename bot")
    ...
    log_file = config.get("log_file", "rename_bot.log")
    logging.basicConfig(
    ...
    bot = RenameBot(config, dry_run=args.dry_run)
    if args.dry_run:
        logging.getLogger("rename_bot").info("Dry-run mode, no approvals will be submitted.")
    bot.run()
